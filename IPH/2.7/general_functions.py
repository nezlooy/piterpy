...
FILETYPES = ['SEARCH_ERROR', 'PY_SOURCE', 'PY_COMPILED', 'C_EXTENSION', 'PY_RESOURCE', 'PKG_DIRECTORY', 'C_BUILTIN', 'PY_FROZEN', 'PY_CODERESOURCE', 'IMP_HOOK']
...
PYTHON_APP_DIR = 'C:\\Python27' # python dist-dir for filtering
...
def pyStringObject_to_string(ea, ob_type='PyStringObject'):
...
def pyUnicodeObject_to_string(ea):
	if isinstance(ea, idaapi.PyIdc_cvt_int64__):
		ea = ea.value
	length = py_object_get_pmember(ea, 'PyUnicodeObject', 'length')
	sval = py_object_get_member(ea, 'PyUnicodeObject', 'str')
	return idc.GetString(sval, length, ASCSTR_UNICODE)
...
def dump_code(ea, uncompyle=True, out=None):
	...
	if uncompyle:
		uncompyle2.uncompyle(sys.version[:3], marshal.loads(dump), out)
...
'''
Condition-code for func "exec_statement" in file "break_condition_exec_statement.py"
Condition-code for func "builtin_eval" in file "break_condition_builtin_eval.py"
Condition-code for func "load_module" in file "break_condition_load_module.py"
